...
def getApplianceData(url):
    ...
    for child in root.findall('meter'):
        ...
        appliance = {'appliance' : str(child.attrib.get('title')), 'energy' : str(child.find('energy').text),
                     'power' : str(child.find('power').text)}
...
def writeCSV():
    ...
    with open('ApplianceData.csv', 'wb') as csvfile:
